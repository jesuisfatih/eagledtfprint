...
/**
 * Company Intelligence Service — Deep company-level analytics
 * Aggregates visitor sessions, orders, carts, and behavioral data per company.
 * Calculates engagement scores, growth trends, department spending, and churn risk.
 */
@Injectable()
export class CompanyIntelligenceService {
  ...
  // ===================================================
  // CALCULATE INTELLIGENCE FOR ALL COMPANIES
  // ===================================================
  async calculateAll(merchantId: string) {
  ...
  // ===================================================
  // CALCULATE INTELLIGENCE FOR A SINGLE COMPANY
  // ===================================================
  async calculateForCompany(merchantId: string, company: any) {
    const companyId = company.id;
    const userIds = company.users?.map((u: any) => u.id) || [];
    ...
    const engagementScore = Math.min(100, Math.round(
      (Math.min(totalSessions, 50) / 50 * 20) +
    ...
    if (totalOrders >= 5 && totalRevenue >= 5000) buyerIntent = 'champion';
    else if (totalOrders >= 3) buyerIntent = 'hot';
    else if (totalAddToCarts >= 3 || totalOrders >= 1) buyerIntent = 'warm';
    else if (totalProductViews >= 5) buyerIntent = 'interested';
    ...
    let segment = 'new';
    if (totalOrders === 0) segment = 'prospect';
    else if (daysSinceLastOrder !== null && daysSinceLastOrder > 180) segment = 'churned';
    else if (daysSinceLastOrder !== null && daysSinceLastOrder > 90) segment = 'at_risk';
    ...
    // Top viewed products
    const productViewCounts = new Map<string, { title: string; count: number }>();
    ...
    if (daysSinceLastOrder !== null) {
      if (daysSinceLastOrder > 180) churnRisk = 0.9;
      else if (daysSinceLastOrder > 90) churnRisk = 0.7;
      else if (daysSinceLastOrder > 60) churnRisk = 0.4;
      else if (daysSinceLastOrder > 30) churnRisk = 0.2;
      else churnRisk = 0.05;
    } else if (totalOrders === 0) {
      churnRisk = 0.5; // Prospect, might never order
    }
  ...
  // ===================================================
  // GET COMPANY INTELLIGENCE DASHBOARD
  // ===================================================
  async getDashboard(merchantId: string) {
    ...
    for (const ci of intelligence) {
      ...
      if (ci.engagementScore > 20) activeCompanies++;
    }
    ...
    // Growth companies (new with high engagement)
  ...
  // ===================================================
  async getCompanyDetail(merchantId: string, companyId: string) {
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      const monthOrders = orders.filter(o =>
        new Date(o.createdAt) >= monthStart && new Date(o.createdAt) <= monthEnd
      );
      monthlyRevenue.push({
